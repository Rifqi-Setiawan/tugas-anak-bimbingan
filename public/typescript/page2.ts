...
let listGenre = document.getElementById("listGenre");

genre?.addEventListener("click", () => {
  listGenre?.classList.toggle("hidden");
  listGenre?.classList.toggle("grid");
});
// genre button end

// katalog buku top start
interface book {
  judul: string;
  image: string;
  penulis: string;
...
let dataBukuTop: book[] = [
  {
    judul: "You do you",
    image:
      "https://cdn.gramedia.com/uploads/picture_meta/2023/6/16/y3sd5cbky6xx4pdkipmeic.jpeg",
  ...
  },
  {
    ...
    image:
      "https://s3-ap-southeast-1.amazonaws.com/ebook-previews/40678/143505/1.jpg",
  ...
  },
  {
    judul: "Almond",
    image:
      "https://cdn.gramedia.com/uploads/items/9786020519807_FIX_Binder_Co.jpg",
  ...
  },
  {
    ...
    image:
      "https://ebooks.gramedia.com/ebook-covers/25313/image_highres/HCO_TKAM2018MTH11.jpeg",
    penulis: "Harper Lee",
    rating: "4.4",
  },
  {
...
// arrow right
let kanan = document.getElementById("arrowRight") as HTMLElement;
...
// arrow left
let kiri = document.getElementById("arrowLeft") as HTMLElement;
...
function bukuTop(
  ...
  let judul = document.querySelectorAll(
    judulTop
  ) as NodeListOf<HTMLParagraphElement>;
  let image = document.querySelectorAll(
    imageTop
  ) as NodeListOf<HTMLImageElement>;
  let penulis = document.querySelectorAll(
    penulisTop
...
// katalog buku top end

// katalog buku bot start
interface books {
    judul: string,
    reads: string,
    rating: string,
...
let urutInner: number = 0;
bookBot(urutInner, 9, dataBukuBot);

function bookBot(urutBot: number, panjang: number, sumberData: books[]): void {
  let generalDiv = document.querySelectorAll('.generalDivBooksBot') as NodeListOf<HTMLElement>;
  let imageBot = document.querySelectorAll('.gambar-bot') as NodeListOf<HTMLImageElement>;
  let judulBot = document.querySelectorAll('.judul-bot') as NodeListOf<HTMLParagraphElement>;
  let readsBot = document.querySelectorAll('.reads-bot') as NodeListOf<HTMLParagraphElement>;
  let ratingBot = document.querySelectorAll('.rating-bot') as NodeListOf<HTMLSpanElement>;
  if (imageBot && judulBot && readsBot) {
    for (let i = 0; i < panjang; i++) {
    let indeks = urutBot + i;
...
function arrow(option: string): void {
...
function getGenre(type: string): void {

  let j: number = 0
  let i: number = 0
  let listGenreBuku: books[] = [];
  if (type === 'all') {
    urutInner = 0
    cek =true
